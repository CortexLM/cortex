...
pub fn log_tail(stdout: &str, secrets: &[String], max_bytes: usize) -> String {
    let body: String = stdout
        ...
        .collect::<Vec<_>>()
        .join("\n");
    prism_lium::truncate_tail(&redact(body.trim(), secrets), max_bytes)
}
...
pub trait EvalPod: Send + Sync {
    ...
    /// Deliver `request` plus the pod env file, run the image, return stdout.
    async fn run(
        &self,
        instance_id: &str,
...
impl LiveScorer for LiumHarvest {
    async fn score(
        ...
        let run = self
            .pod
            .run(&instance, &request, &self.teacher.env_file(pin))
            .await;
...
mod tests {
    ...
    impl EvalPod for FakePod {
        ...
        async fn run(
            ...
            log.env_files.push(env_file.to_owned());
            drop(log);
