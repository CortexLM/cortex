...
mod tests {
    ...
    #[tokio::test]
    async fn report_reads_fail_closed_when_operator_auth_is_unconfigured() {
        let token = "op-test-token";
        ...
        .await;
        assert_eq!(st, StatusCode::SERVICE_UNAVAILABLE, "{list}");
        assert_eq!(list["error"], "auth_unconfigured");
        let dumped = list.to_string();
        assert!(!dumped.contains("repro_steps"), "{dumped}");
        assert!(!dumped.contains("account_id"), "{dumped}");
        assert!(!dumped.contains("miner_hotkey"), "{dumped}");
