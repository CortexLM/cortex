...
//!   paid inference, green → spend token → evaluate → `custom_value`. It is
//!   wired through `proof_eval::FamilyMux` so no custom topic ever falls
//!   back to the digest-pinned harvest.
