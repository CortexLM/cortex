...
7. **Bounty — pair + report:** `ctx bounty pair --hotkey <ss58> --account-id <id> --accept-terms` (`bins/ctx`; `cortex-bounty` is superseded), which itself does `POST /v1/pair` (terms + signature), then `ctx bounty report` → `POST /v1/reports`. Operator bearer `POST /v1/admin/adjudicate` (`valid` / `already_fixed_not_prod` / `invalid_malicious` / `duplicate`). Scoring **reads** CortexLM/backend public JSON (`BOUNTY_BACKEND_PUBLIC_URL`); do not serve `/v1/public/*` from this repo.
...
| Miner CLI (`ctx`) | [`bins/ctx`](bins/ctx) · installer [`scripts/install-ctx.sh`](scripts/install-ctx.sh) · release lane [`.github/workflows/release-ctx.yml`](.github/workflows/release-ctx.yml) |
