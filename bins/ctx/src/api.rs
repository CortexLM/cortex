...
//! `/challenge/{challenge_id}/v1/...`, so one base URL covers all four live
//! challenges.
...
pub struct Reply {
    /// HTTP status code.
    pub status: u16,
    /// Decoded body, or `{"raw": "..."}` when the body was not JSON.
    pub body: Value,
}
...
mod tests {
    ...
    fn challenge_paths_are_proxy_paths() {
        assert_eq!(
            challenge_path("relearn-image", "/v1/status"),
            "/challenge/relearn-image/v1/status"
        );
    }
