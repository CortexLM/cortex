...
pub fn private_file(path: &Path, maximum: u64) -> Result<String, &'static str> {
    String::from_utf8(private_bytes(path, maximum)?).map_err(|_| "invalid private experiment file")
}
...
pub fn private_bytes(path: &Path, maximum: u64) -> Result<Vec<u8>, &'static str> {
    if !path.is_absolute() || fs::canonicalize(path).ok().as_deref() != Some(path) {
        return Err("unsafe private experiment file");
    }
    let uid = rustix::process::geteuid().as_raw();
    let parent = path.parent().ok_or("unsafe private experiment directory")?;
    for (depth, directory) in parent.ancestors().enumerate() {
        ...
        if !stat.is_dir()
            ...
            || writable_ancestor
        {
            return Err("unsafe private experiment directory");
        }
    }
    ...
    let file = File::from(
        ...
        .map_err(|_| "private experiment file unavailable")?,
    );
    let stat = file
        .metadata()
        .map_err(|_| "private experiment file unavailable")?;
    ...
    file.take(maximum.saturating_add(1))
        ...
        .map_err(|_| "invalid private experiment file")?;
    if bytes.is_empty() || bytes.len() as u64 > maximum {
        return Err("invalid private experiment file");
    }
    Ok(bytes)
}
