...
interface RetainedRlmChild {
	session: AgentSession;
	run?: RlmChildRun;
	restoredState?: { status: "done" | "error"; error?: string };
}
...
export class AgentSession {
	...
	private readonly _includeHarnessState: boolean;
	private readonly _allowSessionCommands: boolean;
	...
	constructor(config: AgentSessionConfig) {
		...
		this._includeHarnessState = config.includeHarnessState ?? true;
		this._allowSessionCommands = config.allowSessionCommands ?? true;
	...
	private _rebuildSystemPrompt(toolNames: string[]): string {
		...
		this._baseSystemPromptOptions = {
			...
			harnessState: this._includeHarnessState ? this._loadMergedHarnessState() : undefined,
	...
	private _normalizeSubmission(
		...
		if (this._allowSessionCommands && policy.parseSessionCommands) {
			const command = parseSessionSlashCommand(text);
	...
	private _autoRefineAllowedForSession(): boolean {
		return this._includeHarnessState && this._rlmDepth === 0 && this._localHarnessStateDir() !== undefined;
	}
	...
	async refine(
		...
		if (!this._includeHarnessState) throw new Error("Harness refinement is disabled for this session");
	...
	/** Called by a trusted host only after accepting delivery to the parent. */
	recordParentReply(): void {
		if (this._rlmDepth === 0) throw new Error("Root session has no parent");
	...
	private _createInlineRlmSubagentRuntime(options: CreateRlmSubagentRuntimeOptions): RlmSubagentRuntime {
		...
		const child = new AgentSession({
			...
			includeHarnessState: this._includeHarnessState,
			allowSessionCommands: this._allowSessionCommands,
			rlmDepth: options.rlmDepth,
	...
	private _buildRlmSubagentList(listedAgents?: AgentSessionMessageListResult): RlmListSubagentsResult {
		...
		for (const [childId, { session: childSession, restoredState }] of this._rlmChildSessions) {
			if (
			...
			subagents.push({
				...
				status: restoredState?.status === "error" ? "error" : "completed",
			});
	...
	registerRlmChildSession(
		...
		session: AgentSession,
		unsubscribe?: () => void,
		restoredState?: RetainedRlmChild["restoredState"],
	): boolean {
		...
		if (!restoredState && this._subagentRuntimeHost?.completeRlmSubagentRuntime?.(childId, session) === false) {
			return false;
		}
		if (this._disposed || this._disposing) {
		...
		this._rlmChildSessions.set(childId, { session, run: this._activeRlmChildRuns.get(childId), restoredState });
		if (unsubscribe) {
	...
	private _rlmChildSnapshotForSession(childId: string, child: AgentSession): RlmChildAgentSnapshot {
		const restoredState = this._rlmChildSessions.get(childId)?.restoredState;
		...
		return {
			...
			status: restoredState?.status ?? "done",
