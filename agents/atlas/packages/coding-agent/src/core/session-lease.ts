...
export class SessionLease {
	...
	assertHeld(): void {
		if (this.released) throw new Error("Session lease was released");
		withLeaseGuard(this.directory, () => {
			if (readLeaseOwner(this.directory)?.token !== this.token) {
				throw new Error("Session lease ownership lost");
			}
		});
	}
