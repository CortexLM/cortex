...
export interface KernelManagerOptions {
	...
	/** Embedders may forbid inheriting controller credentials. Default true. */
	inheritEnv?: boolean;
	sessionId?: string;
...
let signalsManagedExternally = false;

/** Dedicated embedders must install and await their own tree-wide shutdown before creating kernels. */
export function manageKernelSignalsExternally(): void {
	if (signalHandlersInstalled) throw new Error("Kernel signal handling is already installed");
	signalsManagedExternally = true;
}
