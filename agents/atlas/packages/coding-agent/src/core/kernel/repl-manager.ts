...
export class ReplKernelManager {
	...
	private wireChild(child: ChildProcess): void {
		...
		child.stdout?.on("data", (buf: Buffer) => {
			...
			if (buffered.length > 16 * 1024 * 1024) {
				this.failProtocolFrame(child, "kernel protocol buffer exceeded 16 MiB");
				return;
			}
	...
	private startHostRequest(requestId: string, data: unknown): void {
		if (this.inFlightHostRequests.size >= 64) {
			this.rejectActiveExecution(new Error("Kernel host request concurrency exhausted"));
			this.disposeSync();
			return;
		}
