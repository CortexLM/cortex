...
export interface IpythonToolOptions {
	...
	/** Set false when the kernel must not inherit controller credentials. */
	inheritEnv?: boolean;
	...
	snapshotDir?: string;
	/** Container-visible snapshot directory when different from the host path. */
	snapshotKernelDir?: string;
		...
		try {
			...
			const m = new ReplKernelManager({
				...
				snapshot: snapshotDir
					? {
							path: snapshotPathIn(this.options?.snapshotKernelDir ?? snapshotDir),
							manifestPath: manifestPathIn(this.options?.snapshotKernelDir ?? snapshotDir),
						}
					: undefined,
			...
			try {
				...
				if (snapshotDir) {
					...
					if (snapshotExisted || restore?.restored.length || restore?.failed.length) {
