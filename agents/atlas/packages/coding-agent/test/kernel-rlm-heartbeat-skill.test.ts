...
describe("RLM heartbeat skill over the kernel host bridge", () => {
	...
	beforeEach(() => {
		...
		// An explicit kernel Python skips skill installation; exercise this checkout without mutating its venv.
		vi.stubEnv(
			"PYTHONPATH",
			[join(bundledRlmHeartbeatSkill().packagePath, "src"), process.env.PYTHONPATH].filter(Boolean).join(delimiter),
		);
	});
...
`);
		expect(unavailable.status, unavailable.error?.traceback.join("\n") ?? unavailable.stderr).toBe("ok");
