...
| `harvest-work/` | Host `debugfs` dump of guest `/work`. If this tree lags the guest overlay (missing trial `result.json` / `verifier/reward.txt` the guest already wrote), harvest refuses rather than publishing `report.json`. Guest writes those files; do not treat a lagging dump as a missing guest `reward.txt`. |
...
- **Incomplete harvest-work dump.** Host reconstruction (`proof-fc-harvest`)
  refuses when `{jail}/harvest-work` is missing trial `result.json` /
  `verifier/reward.txt` that the guest overlay has (n15: guest finished 6/6
  including atrx `reward.txt=0`; the dump's atrx verifier was empty). Do not
  publish that partial copy. Guest already writes `reward.txt`; this is not
