...
| `model_pin` (top-level `constraints.model_pin`) | Exported to the adaptor as `PROOF_MODEL_PIN`. Canon shape is `vendor/model[:tag]` only (`proof-canon` rejects `a/b/c`) | `vendor/model[:tag]` |
