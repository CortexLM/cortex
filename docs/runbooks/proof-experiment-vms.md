...
| `model_pin` (top-level `constraints.model_pin`) | Exported to the adaptor as `PROOF_MODEL_PIN`. Harbor `-m` uses the full OpenRouter / LiteLLM id (`PROOF_PARAM_MODEL` or `openrouter/` + pin when BYOK is `OPENROUTER_API_KEY`) | `vendor/model[:tag]` or `openrouter/vendor/model[:tag]` |
