...
| **503** missing / invalid evaluate `results.json` | Custom evaluate: no topic-defined results JSON, or it does not bind the scored `primary_value` / `claim_holds` / identities. Default text is `adaptor wrote no results.json` — **not** automatically pin/runner skew. Rebake guidance is added only when the **selected** guest runner lacks `write_results_next_to_report`. Pathless miss after a host-attested Done (`CustomRunReport.results` absent) means tip the orch past CustomRunReport.results / #294 harvest attach, not rebake | no | no (guest torn down) |
...
`POST /v1/submissions` (201) and `GET /challenge/proof/v1/submissions/{id}`
expose the same object as `results`. The operator artefact zip always
embeds it at the zip root as `results.json` (even when the topic pinned
another write name). A topic pin is one safe `*.json` segment (8–64
chars, no `/`, no leading `.`); a traversal pin is fail-closed before
any write.
