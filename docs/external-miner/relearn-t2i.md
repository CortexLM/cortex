...
`GET /challenge/relearn-t2i/v1/prompts` publishes the **public split**: the
...
| **No contamination** | If an eval prompt id shows up in your training metadata, the submission is rejected |
...
```bash
curl -sS -X POST https://<gateway>/challenge/relearn-t2i/v1/submissions \
...
`train_prompt_ids` is the bench ids your training mix touched. Declare them
honestly: the contamination gate only rejects overlap with the scored split, and
an undeclared overlap that the harness finds anyway is worse for you.

`claimed_output_hashes` is what the seed-replay gate checks. Include at least
the cells the eval asks for; more is fine.

Poll `GET /challenge/relearn-t2i/v1/submissions/{id}`. Eligible runs sit at
`awaiting_admin` until an operator promotes. You do not promote.

`GET /challenge/relearn-t2i/v1/status` reports the live pins, the judge
