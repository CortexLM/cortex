...
| `400` `artifact required` / `artifact exceeds 5 MiB` / digest mismatch / not a tar / gzip | Custom / `tbench`: no upload and no URI, oversize tar, digest ≠ file, or compressed / non-tar bytes | `ctx proof submit --artifact recipe.tar` (≤5 MiB uncompressed). URI-only still works as compat |
