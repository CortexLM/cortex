...
| `400` `artifact required` / `artifact exceeds 5 MiB` / digest mismatch | Custom / `tbench`: no upload and no URI, oversize tar, or sha256 ≠ file | `ctx proof submit --artifact recipe.tar` (≤5 MiB uncompressed). URI-only still works as compat |
