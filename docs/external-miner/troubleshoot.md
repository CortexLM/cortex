...
| `tbench` `rejected` + `failed_ids` naming `no_eval_short_circuit` / `no_tb4_hardcoding` | Inspect ticked those rules. Evidence that quotes the **rule id** as the marker is a false positive (naming the rule in a README is not a cheat). Real fails quote `skip_eval`, `skip_verifier`, `always_pass_eval`, `short_circuit_eval`, `tb4_answers`, or `hardcoded_tb4` | `tbench` is Proof Firecracker inspect, not Lium. Prompt-only `terminus-2` subclass is allowed. Re-submit after the host ships the scan that ignores rule-id text |
