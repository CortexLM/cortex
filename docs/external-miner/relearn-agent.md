...
Declare at least one of `train_episode_ids`, `train_observation_hashes`, or
`train_environment_ids`. The contamination gate only rejects *overlap* with the
scored set, but a manifest that declares nothing gives it nothing to check and
fails as `contamination_evidence_missing`.

Poll `GET /challenge/relearn-agent/v1/submissions/{id}`. Eligible runs sit at
`awaiting_admin` until an operator promotes. You do not promote.

`GET /challenge/relearn-agent/v1/status` reports the base pin, the episode
commitment and size, and the three arms the eval must run. It never reveals
