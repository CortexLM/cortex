...
| `constraints.task_slice` | Opaque label the runner interprets; the control plane does not. The Harbor reference adaptor resolves it **through the pinned pack** (`slices/<label>.json` / `.txt` or `filter.json` → `slices`). **A label the pack does not define is a refusal**, never informational: a topic that set it asserted a set, and scoring a different one is the LIVE Gate 1 fail-open. The supported escape is to name the set explicitly in `constraints.params.tasks` (with `n_tasks` to bound it), which the guest reads **instead of** the label — a stale label beside an explicit set is not resolved at all. No label has a compiled meaning |
