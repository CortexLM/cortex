...
- No invented topics, leaderboard names or hotkeys. The only topic document
  shown is the documented operator example `dt-no-ib-v0`, framed
  “EXAMPLE · DRAFT · NOT LIVE”. Every table that would list live rows is
  drawn in its empty state.
...
|-------|------|------|----------|
| 01 · Home · 1440 | `2UF-0` | 1440 × 3909 | glass nav; ruled-paper hero; emission ledger 80/20 + **interim burn UID0** callout; two live challenges rows; research loop; how to mine (`ctx`, BYOK note); live vs draft ledger; green footer |
| 02 · Proof · 1440 | `31A-0` | 1440 × 5949 | header + read-only host status (`can_score false`, digest pinned, `open_topics 0`, `baseline_sealed false`, `live_harvest_wired false`); open-topics table **empty state**; topic anatomy + example document (draft, not live); submit flow + required POST JSON; digest-pinned RLM judge, verdict fields, cheat codes; `wta` vs `discovery`; 400 vs 503 table; BYOK callout |
| 03 · Bounty · 1440 | `3B9-0` | 1440 × 3565 | header + quota panel; pair → report → adjudicate; blocking terms; outcomes table (valid+severity, unpriced valid, already_fixed, malicious, duplicate); precision × impact with triage noise off the visible score; public leaderboard **empty, informational**, fail-closed scorer note |
...
| 06 · Status · 1440 | `3M3-0` | 1440 × 2569 | weights panel `sealed: false · burn uid 0 = 100%` + **interim burn UID0** callout; emission shares 80/20 as configuration; five gateway endpoints read live; network metagraph summary with `—` (0 when unknown, TAO price never shown); empty activity |
| 07 · Nav + Footer · components | `3VJ-0` | 1440 × 1693 | nav default, scrolled-over-table (glass), hover + focus, dark, 768, 390; footer band spec |
| 01 · Home · 1440 · Dark | `40F-0` | 1440 × 3909 | token-swapped twin |
| 02 · Proof · 1440 · Dark | `4BS-0` | 1440 × 5949 | token-swapped twin |
...
| Board | Node | Size |
|-------|------|------|
| 01 · Home · 768 | `4LM-0` | 768 × 2484 |
| 01 · Home · 390 | `4LN-0` | 390 × 2191 |
| 02 · Proof · 768 | `4R6-0` | 768 × 1890 |
| 02 · Proof · 390 | `4R7-0` | 390 × 1902 |
...
| Board | Node | Size |
|-------|------|------|
| Console · ctx status + verdict envelope (read-only) | `5D4-0` | 1440 × 926 — `ctx status` mirror and the submission verdict envelope shape, labelled “shape only · no row exists today” |
...
No raster is committed here; the Paper file is the artefact.
