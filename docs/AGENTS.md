...
| [`runbooks/proof-experiment-smoke.md`](runbooks/proof-experiment-smoke.md) | Proof single-task smoke: run one task of any custom-family topic through the real guest agent + adaptor (`deploy/scripts/proof-experiment-smoke.py`: agent / exec / orch drivers, `--dry-run`), the Cursor cloud ↔ metal SSH hop (`deploy/scripts/proof-metal-smoke.sh`), expected output, the `tbench-x0032` RCA, and the live-topic re-sign params |
