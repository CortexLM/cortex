...
| [`runbooks/proof-experiment-vms.md`](runbooks/proof-experiment-vms.md) | Proof experiment VMs: one Firecracker VM per paid job for topics whose signed params select an in-guest runner — generic params, resource caps (default 4 vCPU / 8 GiB, topic override up to 8 / 16, disk ≥ 16 GiB), pack staging, guest image bake (`deploy/guest/bake-rootfs.sh`, rootless podman), RE-LOCK steps, fail-closed rows, limitations |
