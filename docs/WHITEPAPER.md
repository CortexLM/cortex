...
| Proof emission | Evaluated research reaches network rewards | Signing/payout helpers exist in [`proof-challenge`](../crates/proof-challenge/src/lib.rs), but [`main.rs`](../bins/proof-challenge/src/main.rs) starts HTTP with epoch `0` and no automatic leaf-emission loop |
