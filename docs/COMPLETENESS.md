...
| Relearn eval image digests | All three live Relearn images are pinned from [`CortexLM/relearn`](https://github.com/CortexLM/relearn): `relearn-eval` `sha256:303c6357…` (PR #2), `relearn-image-eval` `sha256:81c40dc6…` and `relearn-agent-eval` `sha256:4db52b13…` (PR #3, `54d3537`). A live host still 503s until harvest + champion baseline are recorded. |
