...
| Eval pin | **done** | `config/relearn-pin.toml` — `eval_image_digest` `sha256:00839671…` + `relearn_git_sha` `822d2729…` (`CortexLM/relearn` PR #2). Digest-only, no floating tag. Do not pin `sha256:303c6357…` — that image printed no `RELEARN_EVAL_OK`. |
...
| Relearn eval image digests | All three live Relearn images are pinned from [`CortexLM/relearn`](https://github.com/CortexLM/relearn): `relearn-eval` `sha256:00839671…` (PR #2, the image that prints `RELEARN_EVAL_OK`), `relearn-image-eval` `sha256:81c40dc6…` and `relearn-agent-eval` `sha256:4db52b13…` (PR #3, `54d3537`). A live host still 503s until harvest + champion baseline are recorded. |
...
| Relearn holdout salt | The committed `holdout_commitment` is the labeled CI / local fixture. It is **not** the live seal. Live scoring refuses that digest unless a private commitment is supplied via `RELEARN_HOLDOUT_COMMITMENT` / `_FILE` (secret store, never git). Production must rotate to a private salt **and** a private catalog ([`../config/CEREMONY.md`](../config/CEREMONY.md)). |
