...
| Miner CLI (`bins/ctx`) | **done** | `ctx bounty pair` signs locally and binds through the public gateway, then `ctx bounty report`. `bins/cortex-bounty` is superseded and prints a deprecation notice. |
