...
| Automatic emission | **v0 loop** | `ProofEmitter` (`PROOF_EMIT_POLL_SECS`, default 120 — same cadence as bounty) pins `E` at the last epoch block, signs `emission_scores` when any lattice is positive, and otherwise covers `E` with `NoScore(ChallengeInternal)` so D24 can seal (Held once this process already scored the epoch). In-memory store; a restart can burn an epoch that had scores. Do not infer on-chain payment from `can_score`. |
