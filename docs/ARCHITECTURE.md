...
| `proof-admin` | Operator CLI for Proof topic installs (dynamic-topics **P0 skeleton**): `topic validate`, `topic install [--dry-run]`, `topic list`, `topic show`. Writes a **disabled** row in `proof_topic`; `topic enable` / `disable` / `seal` exit 3 (not implemented in this slice). No route, allocator, or scoring path reads it yet |
...
| `db` | Postgres persistence (bundles, evidence, dissent, challenge tables, `proof_topic` install registry) |
