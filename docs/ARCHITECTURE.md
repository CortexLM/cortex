...
| `proof-challenge` | **Master-only:** signed topics, holdout loading, evaluation orchestration. Library payout is a sum of WTA/discovery topic masses; `ProofEmitter` polls every `PROOF_EMIT_POLL_SECS` (default 120), signs exact-`E` leaves when someone scored, otherwise covers `E` with `ChallengeInternal` |
