...
- [ ] Relearn live harvest: `LIUM_API_KEY` + `LIUM_SSH_PUBLIC_KEY_FILE` present and never committed; `/v1/status` reports `live_harvest_wired: true`. The harvest request carries the holdout to a miner-paid pod — accept that exposure knowingly, keep termination verification on, and rotate salt + catalog on suspicion (`docs/RELEARN.md` § Eval image contract).
