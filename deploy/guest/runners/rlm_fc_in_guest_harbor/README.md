...
| `script` | Miner executable relative to the artefact. Docker, **filtered** tasks, and BYOK are already set. The script sees `$PROOF_TASKS` (and `$PROOF_PACK_DIR/<tasks_dir>` rebound to that copy). `harbor run --path` is rewritten onto the filtered tree so a miner that names `$PROOF_PACK_DIR/tasks` cannot evaluate the unfiltered pack. Summarize also **drops trial names** that are not directories under `$PROOF_TASKS` (a same-uid script can still invoke Harbor; excluded ids are not paid). (`verifier_result.rewards.reward` or `verifier/reward.txt`) — a postamble TypeError / nonzero exit does not discard them. A miner-authored `$PROOF_OUTPUT_DIR/report.json` is deleted and ignored; fail-closed only when `n_measured == 0`. Never wrapped as `terminus-2`. |
